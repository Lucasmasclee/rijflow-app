...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const filePath = join(process.cwd(), 'src', 'app', 'dashboard', 'ai-schedule', 'generated', 'sample_input.json')
      writeFileSync(filePath, JSON.stringify(sampleInput, null, 2), 'utf8')
      console.log('Sample input file saved to:', filePath)
    } catch (error) {
    ...
    return NextResponse.json({
      success: true,
      data: sampleInput,
      message: 'Sample input file generated successfully'
    })
